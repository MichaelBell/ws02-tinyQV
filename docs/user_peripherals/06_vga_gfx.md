...
This peripheral is designed to allow a 4 colour (3 plus black) VGA display at 256x120 resolution, although vertical resolutions of 480 and 240 are theoretically supported if you can drive it fast enough.
...
| 0x2 | Y_LOW  | R    | The row mod 60.  Halfword access to Y_LOW and Y_HIGH together is supported. |
| 0x3 | Y_HIGH  | R    | The row divided by 60. |
...
| ---- | ---- | ------- |
| 1-0  | Y_MASK | Interrupt is generated when (row | Y_MASK) == 0x3.  Set to 0x3 to interrupt every line, set to 0 to interrupt every 4th line. |
| 3-2  | X_OFFSET | The position in the line to generate the interrupt, see below. |

| X_OFFSET | Meaning |
| -------- | ------- |
| 0        | Interrupt at x == 512 (end of line). |
| 1        | Interrupt at x == 128. |
| 2        | Interrupt at x == 256. |
| 3        | Interrupt at x == 384. |
